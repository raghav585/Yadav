...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7926246903:AAFVA-9bpMyD6HXsqYX7uiuHHt6jOqOUNDo") 
...
async def add_channel(client, message: Message):
    ...
    if not any(user[0] == user_id for user in subscription_data):
        await message.reply_text(
            ...
            "🔑 Please contact my admin at: **@Assigner_baba** for subscription details."
        )
        return
...
ADMIN_ID = 7348559049 # Admin ID for restricting the command
...
def read_subscription_data():
    """
    Reads subscription data from the text file.
    Returns a list of subscriptions or an empty list if the file doesn't exist or an error occurs.
    """
...
def read_channels_data():
    """
    Reads channel data from the JSON file.
    Returns the data as a dictionary or an empty list if the file doesn't exist or an error occurs.
    """
...
def write_subscription_data(data):
    """
    Writes subscription data to the text file.
    Data should be a list of lists where each sublist represents a user.
    """
    try:
...
def write_channels_data(data):
    """
    Writes channel data to the JSON file.
    Data should be a dictionary containing channel information.
    """
    try:
...
async def upload_handler(client: Client, m: Message):
    ...
    editable = await m.reply_text('𝐓𝐨 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐀 𝐓𝐱𝐭 𝐅𝐢𝐥𝐞 𝐒𝐞𝐧𝐝 𝐇𝐞𝐫𝐞 ⚡️')
                ...
                # Handling image formats (JPEG, PNG, etc.)
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            copy = await bot.send_photo(chat_id=m.chat.id, photo=f'{name}.jpg', caption=cc2)
                            count += 1
